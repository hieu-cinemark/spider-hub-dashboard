...
export default function LogViewer({ kind, sourceLabel }: { kind: "spider-hub" | "ingest"; sourceLabel: string }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div
        ref={containerRef}
        className="h-[480px] overflow-auto rounded-lg bg-[#0b0f19] p-3 font-mono text-[12px] leading-6 text-[#d6e2f0]"
      >
        {parsedLines.map((line, i) => (
          ...
          >
            {line.timestamp && (
              <span className="shrink-0 text-[#6b7a90]">{line.timestamp}</span>
            )}
            {line.level && (
              <Tag color={LEVEL_COLOR[line.level]} className="!m-0 shrink-0 uppercase">
                {line.level}
              </Tag>
            )}
