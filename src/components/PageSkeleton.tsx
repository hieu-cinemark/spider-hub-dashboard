    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="overflow-hidden rounded-2xl border border-[var(--line)] bg-[var(--card)]">
          <SkelBlock className="aspect-video w-full rounded-none" />
          <div className="flex flex-col gap-2 p-3">
            <SkelBlock className="h-3.5 w-full" />
            <SkelBlock className="h-3.5 w-4/5" />
            <div className="mt-1 flex items-center gap-2">
              <SkelBlock className="h-7 w-7 rounded-full" />
              <SkelBlock className="h-3 w-24" />
            </div>
          </div>
        </div>
